...
class SDKServer {
  ...
  async verifySession(
    ...
    try {
      ...
      let { openId, appId, name, email } = payload as Record<string, unknown>;

      if (!ENV.isProduction) {
        if (!isNonEmptyString(appId)) {
          appId = ENV.appId || ENV.googleClientId || "local";
        }
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    // If using Google OAuth, hydrate from session instead of external endpoints.
    if (!user && ENV.googleClientId) {
      await db.upsertUser({
        openId: sessionUserId,
